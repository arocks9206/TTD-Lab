...
def test_add_friend
  friends_array_current_length = @person2[:friends].length + 1
  result = add_friend(@person2, "Scrappy-Doo")
  assert_equal(friends_array_current_length, result.length)
end
  ...
  def test_remove_friend
    current_friend_length = @person4[:friends].length - 1
    result = remove_friend(@person4, "Daphne")
    assert_equal(current_friend_length, result.length)
  end
